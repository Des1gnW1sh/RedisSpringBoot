...
public class ProcessDiagramGenerator {
	...
	static final Logger logger = Logger
			.getLogger(ProcessDiagramGenerator.class);
	...
	static {
		...
		activityDrawInstructions.put("startEvent",
				new ActivityDrawInstruction() {
					...
					public void draw(
							ProcessDiagramCanvas processDiagramCreator,
		...
		// signal catch
		activityDrawInstructions.put("intermediateSignalCatch",
				new ActivityDrawInstruction() {

					public void draw(
							ProcessDiagramCanvas processDiagramCreator,
							ActivityImpl activityImpl) {
						processDiagramCreator.drawCatchingSignalEvent(
								activityImpl.getX(), activityImpl.getY(),
								activityImpl.getWidth(),
								activityImpl.getHeight());
					}
		...
		// signal throw
		activityDrawInstructions.put("intermediateSignalThrow",
				new ActivityDrawInstruction() {

					public void draw(
							ProcessDiagramCanvas processDiagramCreator,
							ActivityImpl activityImpl) {
						processDiagramCreator.drawThrowingSignalEvent(
								activityImpl.getX(), activityImpl.getY(),
								activityImpl.getWidth(),
								activityImpl.getHeight());
					}
		...
		activityDrawInstructions.put("endEvent", new ActivityDrawInstruction() {
			...
			public void draw(ProcessDiagramCanvas processDiagramCreator,
					ActivityImpl activityImpl) {
		...
		// error start event
		activityDrawInstructions.put("errorStartEvent",
				new ActivityDrawInstruction() {

					public void draw(
							ProcessDiagramCanvas processDiagramCreator,
							ActivityImpl activityImpl) {
						processDiagramCreator.drawErrorStartEvent(
								activityImpl.getX(), activityImpl.getY(),
								activityImpl.getWidth(),
								activityImpl.getHeight());
					}
		...
		activityDrawInstructions.put("businessRuleTask",
				new ActivityDrawInstruction() {

					public void draw(
							ProcessDiagramCanvas processDiagramCreator,
							ActivityImpl activityImpl) {
						processDiagramCreator.drawBusinessRuleTask(
								(String) activityImpl.getProperty("name"),
								activityImpl.getX(), activityImpl.getY(),
								activityImpl.getWidth(),
								activityImpl.getHeight());
		...
		activityDrawInstructions.put("boundarySignal",
				new ActivityDrawInstruction() {
		...
		activityDrawInstructions.put("subProcess",
				new ActivityDrawInstruction() {
					...
					public void draw(
						...
						Boolean isTriggeredByEvent = (Boolean) activityImpl
								.getProperty("triggeredByEvent");
	...
	protected interface ActivityDrawInstruction {
		void draw(ProcessDiagramCanvas processDiagramCreator,
				ActivityImpl activityImpl);
	}
	 ...
	 * @Version: [v2.0.0]
	 */
	protected static ProcessDiagramCanvas generateDiagram(
			ProcessDefinitionEntity processDefinition,
			List<String> highLightedActivities) {
		...
		if (processDefinition.getParticipantProcess() != null) {
			ParticipantProcess pProc = processDefinition
					.getParticipantProcess();
			processDiagramCanvas.drawPoolOrLane(pProc.getName(), pProc.getX(),
					pProc.getY(), pProc.getWidth(), pProc.getHeight());
		}
				...
				if (laneSet.getLanes() != null && laneSet.getLanes().size() > 0) {
					for (Lane lane : laneSet.getLanes()) {
						processDiagramCanvas.drawPoolOrLane(lane.getName(),
								lane.getX(), lane.getY(), lane.getWidth(),
								lane.getHeight());
					}
				}
			}
		...
		for (int index = 1; index <= highLightedActivities.size(); index++) {
			for (ActivityImpl activity : processDefinition.getActivities()) {
				if (highLightedActivities.get(index - 1).equals(
						activity.getId())) {
					drawActivityFlowHighLight(processDiagramCanvas, activity,
							highLightedActivities, index);
					// 最后一个节点红色显示
	 ...
	 * @Version: [v2.0.0]
	 */
	protected static void drawActivityFlowHighLight(
			ProcessDiagramCanvas processDiagramCanvas, ActivityImpl activity,
			List<String> highLightedActivities, int index) {
		...
		for (PvmTransition sequenceFlow : activity.getOutgoingTransitions()) {
			...
			logger.info("流程线[" + sequenceFlow.getId() + "]对应的后续节点ID=["
			...
			if (highLightedActivities.contains(lastActivityImpl.getId())
				...
				if (index >= highLightedActivities.size()) {
					...
					if (lastActivityImpl.getId().equals(
							highLightedActivities.get(index))) {
				...
				if (i < waypoints.size() - 2) {
					...
					if (isHighLightedFlow) {
						processDiagramCanvas
								.drawHighLightSequenceflowWithoutArrow(
										waypoints.get(i - 2),
					...
					// 绘制一条流程线中带箭头的直线部分
					if (isHighLightedFlow) {
						processDiagramCanvas.drawHighLightSequenceflow(
	 ...
	 * @Version: [v2.0.0]
	 */
	protected static void drawActivity(
			ProcessDiagramCanvas processDiagramCanvas, ActivityImpl activity,
			List<String> highLightedActivities) {
		// 获取节点类型
		...
		if (drawInstruction != null) {
			...
			processDiagramCanvas.drawActivityMarkers(activity.getX(),
					activity.getY(), activity.getWidth(), activity.getHeight(),
		...
		for (PvmTransition sequenceFlow : activity.getOutgoingTransitions()) {
			...
			for (int i = 2; i < waypoints.size(); i += 2) { // waypoints.size()
				...
				if (i < waypoints.size() - 2) {
					// 绘制一条流程线中不带箭头的直线部分
					processDiagramCanvas.drawSequenceflowWithoutArrow(
							waypoints.get(i - 2), waypoints.get(i - 1),
							waypoints.get(i), waypoints.get(i + 1),
							drawConditionalIndicator);
				} else {
					processDiagramCanvas.drawSequenceflow(waypoints.get(i - 2),
							waypoints.get(i - 1), waypoints.get(i),
	...
	private static void drawHighLight(
			ProcessDiagramCanvas processDiagramCanvas, ActivityImpl activity) {
		processDiagramCanvas.drawHighLight(activity.getX(), activity.getY(),
	 ...
	 * @Version: [v2.0.0]
	 */
	private static void drawCurrentActivityRed(
			ProcessDiagramCanvas processDiagramCanvas, ActivityImpl activity) {
		processDiagramCanvas.drawHighLightRed(activity.getX(), activity.getY(),
				activity.getWidth(), activity.getHeight());
