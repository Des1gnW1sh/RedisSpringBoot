...
package com.scnjwh.intellectreport.modules.act.rest.diagram.services;
...
public class BaseProcessDefinitionDiagramLayoutResource {
	...
	public ObjectNode getDiagramNode(String processInstanceId,
		...
		if (processDefinition.getLaneSets() != null
				&& !processDefinition.getLaneSets().isEmpty()) {
		...
		for (ActivityImpl activity : processDefinition.getActivities()) {
			getActivity(processInstanceId, activity, activityArray,
					sequenceFlowArray, processInstance, highLightedFlows,
					subProcessInstanceMap);
		}
	...
	private void getActivity(String processInstanceId, ActivityImpl activity,
			ArrayNode activityArray, ArrayNode sequenceFlowArray,
			ProcessInstance processInstance, List<String> highLightedFlows,
		...
		if ("callActivity".equals(properties.get("type"))) {
			...
			if (callActivityBehavior != null) {
				...
				if (processInstanceArray.size() == 0
						&& StringUtils.isNotEmpty(callActivityBehavior
		...
		// Nested activities (boundary events)
		for (ActivityImpl nestedActivity : activity.getActivities()) {
			getActivity(processInstanceId, nestedActivity, activityArray,
					sequenceFlowArray, processInstance, highLightedFlows,
					subProcessInstanceMap);
		}
	}
