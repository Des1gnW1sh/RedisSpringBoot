...
import com.scnjwh.intellectreport.common.security.Digests;
...
import com.scnjwh.intellectreport.common.service.ServiceException;
...
import com.scnjwh.intellectreport.modules.sys.utils.LogUtils;
...
public class SystemService extends BaseService implements InitializingBean {
	...
	public User findUid(String uid) {
		return this.userDao.findUid(uid);
	}
	...
	public void afterPropertiesSet() throws Exception {
		...
		if (isSynActivitiIndetity) {
			...
			if (groupList.size() == 0) {
				Iterator<Role> roles = roleDao.findAllList(new Role())
						.iterator();
				while (roles.hasNext()) {
			...
			if (userList.size() == 0) {
				Iterator<User> users = userDao.findAllList(new User())
						.iterator();
				while (users.hasNext()) {
