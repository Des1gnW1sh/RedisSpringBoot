package com.scnjwh.intellectreport.modules.act.service.creator;
...
public abstract class RuntimeActivityCreatorSupport {
	...
	protected ActivityImpl cloneActivity(
	...
	protected ActivityImpl createActivity(ProcessEngine processEngine,
		...
		UserTaskActivityBehavior cloneActivityBehavior = new UserTaskActivityBehavior(
				null, taskDefinition);
	...
	protected ActivityImpl createActivity(ProcessEngine processEngine,
		...
		ActivityImpl prototypeActivity = ProcessDefUtils.getActivity(
				processEngine, processDefinition.getId(), prototypeActivityId);
	...
	protected void createActivityChain(List<ActivityImpl> activities,
		...
		for (int i = 0; i < activities.size(); i++) {
			...
			activities
					.get(i)
					.createOutgoingTransition("flow" + (i + 1))
					.setDestination(
							i == activities.size() - 1 ? nextActivity
									: activities.get(i + 1));
		}
	}

	protected String createUniqueActivityId(String processInstanceId,
