package com.scnjwh.intellectreport.modules.act.service.cmd;
...
public class ModelDeployProcessDefinitionCmd implements Command<java.lang.Void> {
	...
	public ModelDeployProcessDefinitionCmd(String modelId, String procDefKey,
			String procDefName) {
	...
	public Void execute(CommandContext commandContext) {
		...
		try {
			...
			if (processDefinition != null) {
				ResourceEntityManager resourceEntityManager = commandContext
						.getResourceEntityManager();
				DeploymentEntity deployment = (DeploymentEntity) repositoryService
						.createDeploymentQuery()
						.deploymentId(processDefinition.getDeploymentId())
						.singleResult();

				// 删除原资源
				resourceEntityManager.deleteResourcesByDeploymentId(deployment
						.getId());
				...
				diagramResource.setName(procDefName + "."
			...
			// 不存在部署一个新的流程 ThinkGem
				...
				repositoryService
						.createDeployment()
						.name(procDefName)
						.addInputStream(procDefName + ".bpmn20.xml",
								new ByteArrayInputStream(bpmnBytes)).deploy();
			}
		} catch (Exception e) {
