...
public class ProcessDiagramCanvas {
	...
	static {
		try {
			...
			BUSINESS_RULE_TASK_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("org/activiti/engine/impl/bpmn/deployer/business_rule.png"));
			TIMER_IMAGE = ImageIO
					.read(ReflectUtil
			...
			SIGNAL_CATCH_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("org/activiti/engine/impl/bpmn/deployer/signal_catch.png"));
			SIGNAL_THROW_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("org/activiti/engine/impl/bpmn/deployer/signal_throw.png"));
		} catch (IOException e) {
		...
		try {
			...
			if (minX >= 0 && minY >= 0) {
				imageToSerialize = processDiagram.getSubimage(minX - 5,
						minY - 5, canvasWidth - minX + 5, canvasHeight - minY
								+ 5);
			}
	...
	public void drawErrorEndEvent(int x, int y, int width, int height) {
		...
		g.drawImage(ERROR_THROW_IMAGE, x + 3, y + 3, width - 6, height - 6,
				null);
	}

	public void drawErrorStartEvent(int x, int y, int width, int height) {
		drawNoneStartEvent(x, y, width, height);
		g.drawImage(ERROR_CATCH_IMAGE, x + 3, y + 3, width - 6, height - 6,
				null);
	}
	...
	// 绘制高亮显示的不带箭头的流程线
	public void drawHighLightSequenceflowWithoutArrow(int srcX, int srcY,
			int targetX, int targetY, boolean conditional) {
	...
	public void drawPoolOrLane(String name, int x, int y, int width, int height) {
		...
		if (name != null && name.length() > 0) {
			...
			g.drawString(truncated, x + 2 + fontMetrics.getHeight(), 3 + y
					+ availableTextSpace - (availableTextSpace - realWidth) / 2);
	...
	public void drawBusinessRuleTask(String name, int x, int y, int width,
			int height) {
		drawTask(name, x, y, width, height);
		g.drawImage(BUSINESS_RULE_TASK_IMAGE, x + 7, y + 7, ICON_SIZE,
				ICON_SIZE, null);
	}
	...
	public void drawCollapsedMarker(int x, int y, int width, int height) {
		...
		Line2D.Double line = new Line2D.Double(rect.getCenterX(),
				rect.getY() + 2, rect.getCenterX(), rect.getMaxY() - 2);
		g.draw(line);
		line = new Line2D.Double(rect.getMinX() + 2, rect.getCenterY(),
				rect.getMaxX() - 2, rect.getCenterY());
		g.draw(line);
	}
		...
		if (collapsed) {
			if (!multiInstanceSequential && !multiInstanceParallel) {
				...
				if (multiInstanceSequential) {
					drawMultiInstanceMarker(true, x + MARKER_WIDTH / 2 + 2, y,
							width, height);
				} else if (multiInstanceParallel) {
					drawMultiInstanceMarker(false, x + MARKER_WIDTH / 2 + 2, y,
							width, height);
				}
			}
		} else {
	...
	public void drawMultiInstanceMarker(boolean sequential, int x, int y,
		...
		if (sequential) {
			g.draw(new Line2D.Double(lineX, lineY, lineX + rectangleWidth,
					lineY));
			g.draw(new Line2D.Double(lineX, lineY + rectangleHeight / 2, lineX
					+ rectangleWidth, lineY + rectangleHeight / 2));
			...
			g.draw(new Line2D.Double(lineX, lineY, lineX, lineY
					+ rectangleHeight));
			g.draw(new Line2D.Double(lineX + rectangleWidth / 2, lineY, lineX
					+ rectangleWidth / 2, lineY + rectangleHeight));
	...
	public void drawHighLightSequenceflow(int srcX, int srcY, int targetX,
			int targetY, boolean conditional) {
		Paint originalPaint = g.getPaint();
		Stroke originalStroke = g.getStroke();
