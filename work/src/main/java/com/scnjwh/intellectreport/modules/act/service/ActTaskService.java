 ...
 */
package com.scnjwh.intellectreport.modules.act.service;
...
import com.scnjwh.intellectreport.modules.act.dao.ActDao;
import com.scnjwh.intellectreport.modules.act.entity.Act;
import com.scnjwh.intellectreport.modules.act.service.cmd.CreateAndTakeTransitionCmd;
import com.scnjwh.intellectreport.modules.act.service.cmd.JumpTaskCmd;
import com.scnjwh.intellectreport.modules.act.service.creator.ChainedActivitiesCreator;
import com.scnjwh.intellectreport.modules.act.service.creator.MultiInstanceActivityCreator;
import com.scnjwh.intellectreport.modules.act.service.creator.RuntimeActivityDefinitionEntityIntepreter;
import com.scnjwh.intellectreport.modules.act.service.creator.SimpleRuntimeActivityDefinitionEntity;
import com.scnjwh.intellectreport.modules.act.utils.ActUtils;
import com.scnjwh.intellectreport.modules.act.utils.ProcessDefCache;
import com.scnjwh.intellectreport.modules.act.utils.ProcessDefUtils;
...
public class ActTaskService extends BaseService {
	...
	public Page<Act> historicList(Page<Act> page, Act act) {
		...
		// 查询列表
		List<HistoricTaskInstance> histList = histTaskQuery.listPage(
		...
		for (HistoricTaskInstance histTask : histList) {
			...
			// System.out.println(histTask.getId()+"  =  "+histTask.getProcessVariables()
			// + "  ========== " + histTask.getTaskLocalVariables());
	...
	public List<Act> histoicFlowList(String procInsId, String startAct,
		...
		for (int i = 0; i < list.size(); i++) {
			...
			// 过滤开始节点前的节点
			if (StringUtils.isNotBlank(startAct)
					&& startAct.equals(histIns.getActivityId())) {
				start = true;
			}
			...
			if (StringUtils.isNotBlank(histIns.getAssignee())
				...
				if (StringUtils.isNotBlank(histIns.getTaskId())) {
					List<Comment> commentList = taskService
							.getTaskComments(histIns.getTaskId());
			...
			// 过滤结束节点后的节点
			if (StringUtils.isNotBlank(endAct)
					&& endAct.equals(histIns.getActivityId())) {
				boolean bl = false;
				Integer actNum = actMap.get(histIns.getActivityId());
				...
				for (int j = i + 1; j < list.size(); j++) {
					...
					if ((actNumA != null && actNumA < actNum)
							|| StringUtils.equals(hi.getActivityId(),
									histIns.getActivityId())) {
						bl = true;
					}
				}
				if (!bl) {
	 ...
	 * 任务后退至指定活动
	 */
	public void taskBack(TaskEntity currentTaskEntity,
			Map<String, Object> variables) {
		ActivityImpl activity = (ActivityImpl) ProcessDefUtils
				.getActivity(processEngine,
						currentTaskEntity.getProcessDefinitionId(),
						currentTaskEntity.getTaskDefinitionKey())
	 ...
	 * 任务前进至指定活动
	 */
	public void taskForward(TaskEntity currentTaskEntity,
			Map<String, Object> variables) {
		ActivityImpl activity = (ActivityImpl) ProcessDefUtils
				.getActivity(processEngine,
						currentTaskEntity.getProcessDefinitionId(),
	 ...
	 * 跳转（包括回退和向前）至指定活动节点
	 */
	public void jumpTask(String procInsId, String targetTaskDefinitionKey,
			Map<String, Object> variables) {
	 ...
	 * 跳转（包括回退和向前）至指定活动节点
	 */
	public void jumpTask(String procInsId, String currentTaskId,
			String targetTaskDefinitionKey, Map<String, Object> variables) {
		jumpTask(getTaskEntity(currentTaskId), targetTaskDefinitionKey,
				variables);
	}
	 ...
	 * @throws Exception
	 */
	public void jumpTask(TaskEntity currentTaskEntity,
			String targetTaskDefinitionKey, Map<String, Object> variables) {
		ActivityImpl activity = ProcessDefUtils.getActivity(processEngine,
				currentTaskEntity.getProcessDefinitionId(),
	 ...
	 * @throws Exception
	 */
	private void jumpTask(TaskEntity currentTaskEntity,
			ActivityImpl targetActivity, Map<String, Object> variables) {
		CommandExecutor commandExecutor = ((RuntimeServiceImpl) runtimeService)
				.getCommandExecutor();
		commandExecutor.execute(new JumpTaskCmd(currentTaskEntity,
				targetActivity, variables));
	}
	...
	public ActivityImpl[] insertTasksAfter(String procDefId, String procInsId,
			String targetTaskDefinitionKey, Map<String, Object> variables,
			String... assignees) {
		...
		ActivityImpl prototypeActivity = ProcessDefUtils.getActivity(
				processEngine, processDefinition.getId(),
				targetTaskDefinitionKey);
		return cloneAndMakeChain(processDefinition, procInsId,
				targetTaskDefinitionKey, prototypeActivity
						.getOutgoingTransitions().get(0).getDestination()
						.getId(), variables, newAssignees);
	}

	/**
	 * 前加签
	 */
	public ActivityImpl[] insertTasksBefore(String procDefId, String procInsId,
			String targetTaskDefinitionKey, Map<String, Object> variables,
			String... assignees) {
		ProcessDefinitionEntity procDef = (ProcessDefinitionEntity) repositoryService
				.getProcessDefinition(procDefId);
		return cloneAndMakeChain(procDef, procInsId, targetTaskDefinitionKey,
	 ...
	 * 分裂某节点为多实例节点
	 */
	public ActivityImpl splitTask(String procDefId, String procInsId,
			String targetTaskDefinitionKey, Map<String, Object> variables,
			String... assignee) {
		return splitTask(procDefId, procInsId, targetTaskDefinitionKey,
				variables, true, assignee);
	}
	...
	public ActivityImpl splitTask(String procDefId, String procInsId,
		...
		RuntimeActivityDefinitionEntityIntepreter radei = new RuntimeActivityDefinitionEntityIntepreter(
				info);
		...
		ProcessDefinitionEntity processDefinition = (ProcessDefinitionEntity) repositoryService
				.getProcessDefinition(procDefId);
		ActivityImpl clone = new MultiInstanceActivityCreator()
				.createActivities(processEngine, processDefinition, info)[0];

		TaskEntity currentTaskEntity = this.getCurrentTask(procInsId);

		CommandExecutor commandExecutor = ((RuntimeServiceImpl) runtimeService)
				.getCommandExecutor();
		commandExecutor.execute(new CreateAndTakeTransitionCmd(
				currentTaskEntity, clone, variables));
	...
	private TaskEntity getTaskEntity(String taskId) {
		return (TaskEntity) taskService.createTaskQuery().taskId(taskId)
				.singleResult();
	}

	@SuppressWarnings("unchecked")
	private ActivityImpl[] cloneAndMakeChain(ProcessDefinitionEntity procDef,
			String procInsId, String prototypeActivityId,
			String nextActivityId, Map<String, Object> variables,
			String... assignees) {
		SimpleRuntimeActivityDefinitionEntity info = new SimpleRuntimeActivityDefinitionEntity();
		...
		ActivityImpl[] activities = new ChainedActivitiesCreator()
				.createActivities(processEngine, procDef, info);
	...
	// private void
	// recordActivitiesCreation(SimpleRuntimeActivityDefinitionEntity info) {
	...
	// */
	// public ActivityImpl split(String targetTaskDefinitionKey, boolean
	// isSequential, String... assignees) throws Exception {
	// SimpleRuntimeActivityDefinitionEntity info = new
	...
	//
	// RuntimeActivityDefinitionEntityIntepreter radei = new
	// RuntimeActivityDefinitionEntityIntepreter(info);
	//
	// radei.setPrototypeActivityId(targetTaskDefinitionKey);
	// radei.setAssignees(CollectionUtils.arrayToList(assignees));
	// radei.setSequential(isSequential);
	//
	// ActivityImpl clone = new
	// MultiInstanceActivityCreator().createActivities(_processEngine,
	// processDefinition, info)[0];
	//
	// TaskEntity currentTaskEntity = getCurrentTask();
	// executeCommand(new
	// CreateAndTakeTransitionCmd(currentTaskEntity.getExecutionId(), clone));
	...
	// }
	//
	// public ActivityImpl split(String targetTaskDefinitionKey, String...
	...
	public InputStream tracePhoto(String processDefinitionId, String executionId) {
		...
		Context.setProcessEngineConfiguration(processEngineFactory
	...
	private Map<String, Object> packageSingleActivitiInfo(
		...
		vars.put("任务类型",
				ActUtils.parseToZhType(properties.get("type").toString()));
	...
	private void setCurrentTaskAssignee(Map<String, Object> vars,
		...
		if (assignee != null) {
			org.activiti.engine.identity.User assigneeUser = identityService
					.createUserQuery().userId(assignee).singleResult();
