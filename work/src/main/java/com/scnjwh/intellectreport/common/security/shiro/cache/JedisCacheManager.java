 ...
 */
package com.scnjwh.intellectreport.common.security.shiro.cache;
...
import com.scnjwh.intellectreport.common.utils.JedisUtils;
import com.scnjwh.intellectreport.common.web.Servlets;
	...
	public class JedisCache<K, V> implements Cache<K, V> {
		...
		public V get(K key) throws CacheException {
			...
			try {
				...
				value = (V) JedisUtils.toObject(jedis.hget(
						JedisUtils.getBytesKey(cacheKeyName),
		...
		public V put(K key, V value) throws CacheException {
			...
			try {
				...
				jedis.hset(JedisUtils.getBytesKey(cacheKeyName),
						JedisUtils.getBytesKey(key), JedisUtils.toBytes(value));
		...
		public int size() {
			...
			try {
				jedis = JedisUtils.getResource();
				size = jedis.hlen(JedisUtils.getBytesKey(cacheKeyName))
						.intValue();
		...
		public Set<K> keys() {
			...
			try {
				jedis = JedisUtils.getResource();
				Set<byte[]> set = jedis.hkeys(JedisUtils
						.getBytesKey(cacheKeyName));
				for (byte[] key : set) {
