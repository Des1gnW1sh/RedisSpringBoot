 ...
 */
package com.scnjwh.intellectreport.common.supcan;
...
import com.scnjwh.intellectreport.common.supcan.annotation.treelist.SupTreeList;
import com.scnjwh.intellectreport.common.supcan.annotation.treelist.cols.SupCol;
import com.scnjwh.intellectreport.common.supcan.annotation.treelist.cols.SupGroup;
...
import com.scnjwh.intellectreport.common.supcan.annotation.treelist.cols.SupCol;
import com.scnjwh.intellectreport.common.supcan.annotation.treelist.cols.SupGroup;
import com.scnjwh.intellectreport.common.supcan.treelist.TreeList;
import com.scnjwh.intellectreport.common.supcan.treelist.cols.Col;
import com.scnjwh.intellectreport.common.supcan.treelist.cols.Group;
import com.scnjwh.intellectreport.common.utils.CacheUtils;
import com.scnjwh.intellectreport.common.utils.SpringContextHolder;
import com.scnjwh.intellectreport.common.utils.StringUtils;
import com.scnjwh.intellectreport.common.web.BaseController;
...
public class SupcanController extends BaseController {
	...
	public TreeList treeList(@PathVariable("typeAlias") String typeAlias) {
		...
		try {
			...
			clazz = sqlSessionFactory.getConfiguration().getTypeAliasRegistry()
					.resolveAlias(typeAlias);
		} catch (Exception e) {
			// 取不到类型，返回空。
			return null;
		}
		...
		for (Method m : clazz.getMethods()) {
			...
			if (supCol != null) {
				...
				if (StringUtils.isBlank(col.getName())) {
					col.setName(StringUtils.uncapitalize(StringUtils.substring(
							m.getName(), 3)));
				}
	 ...
	 * @param parentId
	 */
	private void listToTree(List<Object> colList, Map<String, Group> groupMap,
			String parentId, Comparator<Object> comparator) {
			...
			if (StringUtils.equals(parentId, g.getParentId())) {
				...
				for (Map.Entry<String, Group> ec : groupMap.entrySet()) {
					...
					if (g.getId() != null && g.getId().equals(gc.getParentId())) {
						...
						listToTree(childrenList, groupMap, gc.getParentId(),
								comparator);
