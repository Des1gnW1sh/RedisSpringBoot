...
public class ThumbnailCommand extends Command
{
  ...
  public void setResponseHeader(HttpServletResponse response, ServletContext sc)
  {
    ...
    String mimetype = getMimeTypeOfImage(sc, response);
		// Set content type  
//		if (mimetype != null) { 
//			response.setContentType(mimetype + ";name='"   + this.fileName + "'");
//		} else {  
//		    response.setContentType("name='" + this.fileName + "'");//这里修改  
//		}  
//		new String(this.fileName.getBytes("ISO-8859-1"),"utf-8") 
    
    if (mimetype != null) {  
        response.setContentType(mimetype);  
    }  
    String value = "attachment; filename=" + this.fileName;  
  ...
  private String getMimeTypeOfImage(ServletContext sc, HttpServletResponse response)
  {
    ...
    String tempFileName = this.fileName.substring(0, this.fileName.lastIndexOf('.') + 1).concat(FileUtils.getFileExtension(this.fileName).toLowerCase());
    ...
    throws ConnectorException
  {
    ...
    try {
      if (!this.thumbFile.exists()) {
        File orginFile = new File(((ResourceType)this.configuration.getTypes().get(this.type)).getPath() + this.currentFolder, this.fileName);
