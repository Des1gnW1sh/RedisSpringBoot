...
var config = {
    ...
    chatlogurl: url+'/basesys/baseSysMsg?userOneId='+currentId+"&userTwoId=",
    aniTime: 200,
    right: -232,
    api: {
	   friend: url+'/basesys/baseSysMsg/friend', //好友列表接口
        group:  url+'/basesys/baseSysMsg/group', //群组列表接口 
        chatlog:  url+'/basesys/baseSysMsg/chatLog', //聊天记录接口
        groups:  url+'/basesys/baseSysMsg/groups', //群组成员接口
        sendurl: '' //发送消息接口
    },
    user: { //当前用户信息
    	id:currentId,
                ...
                +'<div class="layim_chatuser">'
                    + function(){
                        if(type === 'me'){
                            ...
                            return  '<span class="layim_chatname">'+ param.name +'</span>'
...
{ 
	 $("ul.xxim_chatlist li").each(function(){//状态还原
		 ...
		 if((','+arra+",").indexOf(','+dataId+',')>=0){
			 ...
			 var dataOneId = $one_li.attr("data-id");
			 if(dataOneId!=dataId){
...
xxim.event = function(){
    ...
    node.list.on('click', 'h5', function(){
        ...
        if(parentss.hasClass('user_down')){
            chat.hide();
            parentss.attr('class','user_right');
        } else {
            chat.show();
            parentss.attr('class','user_down');
        }
    });
    ...
    config.json(config.api.friend, {}, function(datas){
        if(datas.status === 1){
            ...
            if(myflen > 1){
                 ...
                 +'<h5><i class="user_down"></i><span class="xxim_parentname">'+ '联系人' +'</span></h5>'
...
xxim.getDates = function(index){
    ...
    config.json(api[index], {}, function(datas){
        if(datas.status === 1&&datas.data!=null){
            var i = 0, myflen = datas.data.length, str = '', item;
            if(myflen > 0){
                if(index !== 2){
                    for(; i < myflen; i++){
                        str += '<li data-id="'+ datas.data[i].id +'" class="xxim_parentnode">'
                            +'<h5><i class="user_right"></i><span class="xxim_parentname">'+ datas.data[i].name +'</span><em class="xxim_nums">（'+ datas.data[i].nums +'）</em></h5>'
            ...
            myf.html('<li class="xxim_errormsg">'+ '正在更新功能' +'</li>');
        }
    }, function(){
        myf.html('<li class="xxim_errormsg">请求失败</li>');
        myf.removeClass('loading');
...
xxim.view = (function(){
    var xximNode = xxim.layimNode = $('<div id="xximmm" class="xxim_main">'
            ...
            +'  <div class="xxim_search"><div class="search"></div><input id="xxim_searchkey" style="border:2px #ccc solid;border-radius:5px;margin-top: 14px;"/><span id="xxim_closesearch" style="margin-top: 7px;">×</span></div>'
            +'  <div class="xxim_tabs" id="xxim_tabs"><span class="xxim_tabfriend" title="好友"><div class="user_list"></div></span><span class="xxim_tabgroup" title="群组"><div class="users_list"></div></span><span class="xxim_latechat"  title="最近聊天"><div class="msg_list"></div></span></div>'
