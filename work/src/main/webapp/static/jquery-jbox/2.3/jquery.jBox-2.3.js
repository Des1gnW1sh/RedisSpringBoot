...
(function(a) {
	a.jBox = function(b, c) {
		...
		if (!c.isTip && !c.isMessager && !c.showScrolling) {
			a(a.browser.msie ? 'body' : 'body').attr('style',
		...
		j.push('<div id="' + c.id + '" class="jbox-'
				+ (c.isTip ? 'tip' : (c.isMessager ? 'messager' : 'body'))
				+ '">');
		...
		if (c.showFade) {
			if ((d && a('iframe').length > 0x0)
					|| a('object, applet').length > 0x0) {
				// j.push('<iframe id="jbox-fade" class="jbox-fade"
				// src="about:blank"
				// style="display:block;position:absolute;z-index:-1;"></iframe>');
				...
				;
				j
						.push('<div id="jbox-fade" class="jbox-fade" style="position:absolute;"></div>');
			}
			// 解决框架内object和applet被遮住问题
		...
		;
		j
				.push('<div id="jbox-temp" class="jbox-temp" style="width:0px;height:0px;background-color:#ff3300;position:absolute;z-index:1984;fdisplay:none;"></div>');
		if (c.draggable) {
			j
					.push('<div id="jbox-drag" class="jbox-drag" style="position:absolute;z-index:1984;display:none;"></div>')
		}
		;
		j
				.push('<div id="jbox" class="jbox" style="position:absolute;width:auto;height:auto;">');
		j
		...
		if (c.border > 0x0) {
			...
			j
					.push('<td class="jbox-border" style="margin:0px;padding:0px;border:none;border-radius:'
							...
							+ 'px 0 0 0;width:'
							+ c.border
							+ 'px;height:' + c.border + 'px;"></td>');
			j
					.push('<td class="jbox-border" style="margin:0px;padding:0px;border:none;height:'
							+ c.border + 'px;overflow: hidden;"></td>');
			j
		...
		j.push('<tr>');
		j
				.push('<td class="jbox-border" style="margin:0px;padding:0px;border:none;"></td>');
		...
		j
				.push('<a class="jbox-close" title="'
						...
						+ (0x6 + c.border) + 'px; right:' + (0x6 + c.border)
						+ 'px; width:15px; height:15px;'
						+ (c.showClose ? '' : 'display:none;') + '"></a>');
		if (g) {
			j.push('<div class="jbox-title-panel" style="height:25px;">');
			j
					.push('<div class="jbox-title'
							+ (c.showIcon == true ? ' jbox-title-icon'
									: (c.showIcon == false ? '' : ' '
											+ c.showIcon))
							+ '" style="float:left; width:'
							+ i
							+ '; line-height:'
							+ (a.browser.msie ? 0x19 : 0x18)
							+ 'px; padding-left:'
							+ (c.showIcon ? 0x12 : 0x5)
		...
		j.push('</div>');
		j.push('</div>');
		var k = '<iframe name="jbox-iframe-'
				+ c.id
				+ '" id="jbox-iframe" width="100%" height="100%" marginheight="0" marginwidth="0" frameborder="0" scrolling="'
				+ c.iframeScrolling + '"></iframe><form id="jbox-iframe-form-'
				+ c.id + '" method="post" target="jbox-iframe-' + c.id
				+ '"></form>';
		var l = a(window);
		...
		if (!f.isObject) {
			...
			case "IFRAME":
				f.html = k
				if (b.indexOf('#') == -0x1) {
					f.url = b + (b.indexOf('?') == -0x1 ? '?___t' : '&___t')
							+ Math.random()
				} else {
					var N = b.split('#');
					f.url = N[0x0]
							+ (N[0x0].indexOf('?') == -0x1 ? '?___t' : '&___t')
							+ Math.random() + '#' + N[0x1]
				}
				;
				break
		...
		var u = a.browser.msie ? 'line-height:19px;padding:0px 6px 0px 6px;'
		...
		a
				.each(
						...
						function(N, O) {
							...
							;
							r
									.push('<div id="jbox-state-'
											+ N
											+ '" class="jbox-state" style="display:none;">');
							r
									.push('<div style="min-width:50px;width:'
											...
											+ ';">'
											+ R
											+ '<div id="jbox-content" class="jbox-content" style="height:'
											+ Q
							...
							;
							a.each(O.buttons, function(T, U) {
								r.push('<button class="jbox-button" value="'
										+ U + '" style="' + u + '">' + T
										+ '</button>')
							});
							r.push('</div></div>')
						});
		o.find('#jbox-states').html(r.join('')).children('.jbox-state:first')
				.css('display', 'block');
		if (h) {
		...
		a.each(b, function(N, O) {
			...
			P.children('.jbox-button-panel').children('button').click(
					function() {
				...
				a(this).addClass('jbox-button-hover')
					...
					'mouseout',
					function() {
						a(this).removeClass('jbox-button-active').removeClass(
								'jbox-button-hover')
					});
			P.find('.jbox-button-panel button:eq(' + O.buttonsFocus + ')')
		...
		var w = function() {
			var N = l.width();
			return document.body.clientWidth < N ? N
					: document.body.clientWidth
		};
		var x = function() {
			var N = l.height();
			return document.body.clientHeight < N ? N
		...
		var B = function() {
			...
			;
			if ((c.showFade && !c.isTip)
					|| (!c.showFade && !c.isTip && !c.isMessager)) {
				n.css({
					position : (d) ? "absolute" : "fixed",
		...
		var F = function(N) {
			...
			if (c.dragLimit) {
				...
				var S = document.documentElement.clientHeight
						- N.data.target.height() - 0x1;
				var T = 0x1;
				var U = document.documentElement.clientWidth
						- N.data.target.width() - 0x1;
				if (Q < R)
		...
		var H = function(N) {
			...
			a(document).bind('mousedown.draggable', P, E).bind(
					'mousemove.draggable', P, F)
					.bind('mouseup.draggable', P, G)
		};
		var I = function() {
			if (!c.isTip && !c.isMessager) {
				...
				if (c.isTip) {
					...
					if (tip && tip.next == true) {
						...
						if (N == o.offset().top) {
							...
							o.find('#jbox-content').html(
									tip.options.content.substr(0x5)).end()
									.css(
											{
												left : ((l.width() - o
														.outerWidth()) / 0x2)
		...
		var J = function() {
			...
			if (c.showFade) {
				p.fadeOut('fast', function() {
					p.unbind('click', y).unbind('mousedown', C).html('')
							.remove()
				})
			}
			;
						...
						.ajax({
							...
							error : function() {
								o
										...
										.html(
												'<div style="padding-top:50px;padding-bottom:50px;text-align:center;">Loading Error.</div>')
							}
						});
				break;
			case "IFRAME":
				o
						...
						.bind(
								...
								function(N) {
									...
									o
											.find(
													'#jbox-states .jbox-state:first .jbox-button-focus')
											.focus();
									L()
								});
				// post方式提交iframe
				...
				if (c.ajaxData != undefined) {
					for ( var x in c.ajaxData) {
						jboxIframeForm.append('<input type="hidden" name="' + x
								+ '" value="' + c.ajaxData[x] + '"/>');
					}
				}
	...
	a.jBox.setDefaults = function(b) {
		...
		a.jBox.stateDefaults = a.extend({}, a.jBox.stateDefaults,
				b.stateDefaults);
		a.jBox.tipDefaults = a.extend({}, a.jBox.tipDefaults, b.tipDefaults);
		a.jBox.messagerDefaults = a.extend({}, a.jBox.messagerDefaults,
				b.messagerDefaults);
		a.jBox.languageDefaults = a.extend({}, a.jBox.languageDefaults,
				b.languageDefaults)
	};
	...
	a.jBox.goToState = function(b, c) {
		...
		if (d != undefined && d != null) {
			...
			if (typeof b == 'string') {
				...
				e = b ? d.find('.jbox-state:visible').next() : d.find(
						'.jbox-state:visible').prev()
			}
			;
	...
	a.jBox.prompt = function(b, c, d, e) {
		...
		;
		if (e.icon != 'info' && e.icon != 'warning' && e.icon != 'success'
				&& e.icon != 'error' && e.icon != 'question') {
			padding = '';
			e.icon = 'none'
		}
		;
		var f = e.title == undefined ? 0xa : 0x23;
		var g = e.icon == 'none' ? 'height:auto;'
				: 'min-height:30px;'
						+ ((a.browser.msie && parseInt(a.browser.version) < 0x7) ? 'height:auto !important;height:100%;_height:30px;'
								: 'height:auto;');
		var h = [];
		h.push('html:');
		h.push('<div style="margin:10px;' + g + 'padding-left:'
				+ (e.icon == 'none' ? 0x0 : 0x28) + 'px;text-align:left;">');
		h.push('<span class="jbox-icon jbox-icon-' + e.icon
				+ '" style="position:absolute; top:' + (f + e.border)
				+ 'px;left:' + (0xa + e.border)
	...
	a.jBox.warning = function(b, c, d, e) {
		var defaults = {
			buttons : eval('({ "' + a.jBox.languageDefaults.yes + '": "yes", "'
					+ a.jBox.languageDefaults.no + '": "no", "'
					+ a.jBox.languageDefaults.cancel + '": "cancel" })')
		};
		if (d != undefined && typeof d == 'function') {
			defaults.submit = d
	...
	a.jBox.tip = function(b, c, d) {
		...
		;
		if (d.icon != 'info' && d.icon != 'warning' && d.icon != 'success'
				&& d.icon != 'error' && d.icon != 'loading') {
			d.icon = 'info'
		}
		;
		var e = [];
		...
		e.push('<span class="jbox-icon jbox-icon-' + d.icon
				+ '" style="position:absolute;top:' + (0x4 + d.border)
				+ 'px;left:' + (0x4 + d.border)
				+ 'px; width:32px; height:32px;"></span>');
		e.push(d.content);
		e.push('</div>');
		d.content = e.join('');
	...
	a.jBox.messager = function(b, c, d, e) {
		...
		var h = e.icon == 'none' ? 'height:auto;'
				: 'min-height:30px;'
						+ ((a.browser.msie && parseInt(a.browser.version) < 0x7) ? 'height:auto !important;height:100%;_height:30px;'
								: 'height:auto;');
		var i = [];
		i.push('html:');
		i.push('<div style="margin:10px;' + h + 'padding-left:'
				+ (e.icon == 'none' ? 0x0 : 0x28) + 'px;text-align:left;">');
		i.push('<span class="jbox-icon jbox-icon-' + e.icon
				+ '" style="position:absolute; top:' + (g + e.border)
				+ 'px;left:' + (0xa + e.border)
				+ 'px; width:32px; height:32px;"></span>');
		i.push(e.content);
		i.push('</div>');
...
jBoxConfig.defaults = {
	...
	top : '10%', /* 窗口离顶部的距离,可以是百分比或像素(如 '100px') */
	...
	ajaxData : {}, /*
					 * 在窗口内容使用get:或post:前缀标识的情况下，ajax post的数据，例如：{ id: 1 } 或
					 * "id=1"
					 */
