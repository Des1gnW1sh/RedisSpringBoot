		...
		<li  class="${requestScope.category.id eq tpl.id ?'menu_activ':''}">
			<c:choose>
				<c:when test="${fn:length(tpl.name) gt 12}">
					<div class="ColumnSec"><span><a href="${url}" target="${tpl.target}" style="line-height:16px;padding-top:3px;">${tpl.name}</a></span></div>
					<%-- <a href="${url}" target="${tpl.target}" style="line-height:16px;padding-top:3px;">${tpl.name}</a> --%>
				</c:when>
				<c:otherwise>
					<div class="ColumnSec"><span><a href="${url}" target="${tpl.target}" ${fn:length(tpl.name) gt 10?'style="font-size:12px;"':''}>${tpl.name}</a></span></div>
