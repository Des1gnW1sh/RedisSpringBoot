...
<body>
	...
	<div id="search" class="form-search hide" style="padding:10px 0 0 13px;">
		<label for="key" class="control-label" style="padding:5px 5px 3px 0;margin-right: 13px;">类型：</label>
		<select  id="typeOf" name="typeOf" class="input-xlarge required" style="width: 67%" onchange="selectType()">
		<option value="" label=""/>
		<c:set var="details" value="${fns:getDictList('sys_office_Type')}" />
		<c:if test="${!empty details}">
			<c:forEach  items ="${details}" var='detail' varStatus="sNub">
				<option value="${detail.value}" label="">${detail.label}</option>
			</c:forEach>
		</c:if>
					
			<%-- <option value="" label=""/>
			<option value="" label=""/>
			<options items="${fns:getDictList('sys_office_Type')}" itemLabel="label" itemValue="value" htmlEscape="false"/> --%>
		</select>
