...
<body>
			...
			<div class="toolbar">		 
				<ul >	
					<shiro:hasPermission name="sys.orgs:sysOrgsFiscal:edit">
						<li class="btn-group">
							<a  class="btn btn-primary" href="javascript:void(0);"onclick="contentSave('${ctx}/sys/area/form','新增',700,350)" ><i class="icon-add"></i>新增</a>
						 </li>
					</shiro:hasPermission>
				</ul>
			</div>	
	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.id}}" pId="{{pid}}">
			<td><a onclick="contentSave('${ctx}/sys/area/form?id={{row.id}}','新增',700,350)">{{row.name}}</a></td>
			<td>{{row.code}}</td>
			<td>{{dict.type}}</td>
			<td>{{row.remarks}}</td>
