		...
		<div class="toolbar">		 
			<ul >	
				<shiro:hasPermission name="sys.orgs:sysOrgsFiscal:edit">
					<li class="btn-group">
						<a  class="btn btn-primary" href="javascript:void(0);"onclick="contentSave('${ctx}/sys/menu/form','新增',700,580)" ><i class="icon-add"></i>新增</a>
					 </li>
				</shiro:hasPermission>
			</ul>
		</div>	
		...
		<table id="treeTable" class="table table-striped table-bordered table-condensed hide">
			...
			<tbody><c:forEach items="${list}" var="menu">
				<tr id="${menu.id}" pId="${menu.parent.id ne '1'?menu.parent.id:'0'}">
					...
					<a onclick="contentSave('${ctx}/sys/menu/form?id=${menu.id}','修改',700,580)">${menu.name}</a>
					</td>
					...
					<shiro:hasPermission name="sys:menu:edit"><td nowrap>
						<a onclick="contentSave('${ctx}/sys/menu/form?id=${menu.id}','修改',700,580)">修改</a>
						<a href="${ctx}/sys/menu/delete?id=${menu.id}" onclick="return confirmx('要删除该菜单及所有子菜单项吗？', this.href)">删除</a>
						<a onclick="contentSave('${ctx}/sys/menu/form?parent.id=${menu.id}','新增',700,580)">添加下级菜单</a> 
					</td></shiro:hasPermission>
				</tr>
			</c:forEach></tbody>
		</table>
		<shiro:hasPermission name="sys:menu:edit"><div class="form-actions pagination-left">
			<input id="btnSubmitSort" class="btn btn-primary" type="button" value="保存排序" onclick="updateSort();"/>
