...
<body>
	<%-- <ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="cms:article:edit"><li><a href="">文章添加</a></li></shiro:hasPermission>
	</ul> --%>
	<form:form id="searchForm" modelAttribute="article" action="${ctx}/cms/article/" method="post" class="breadcrumb form-search">
		...
		<shiro:hasPermission name="cms:article:edit">
			<a  class="btn btn-primary"    href="javascript:void(0);"onclick="contentSave('<c:url value='${fns:getAdminPath()}/cms/article/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>','新增',1200,730)" ><i class="icon-add"></i>新增</a>
		</shiro:hasPermission>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="article">
			<tr>
				...
				<td><a href="javascript:void(0);"onclick="contentSave('${ctx}/cms/article/form?id=${article.id}','修改',1200,730)">${fns:abbr(article.title,40)}</a></td>
				...
				<td>
					...
					<shiro:hasPermission name="cms:article:edit">
						...
						<a href="javascript:void(0);"onclick="contentSave('${ctx}/cms/article/form?id=${article.id}','修改',1200,730)" ><i class="icon-add"></i>修改</a>
