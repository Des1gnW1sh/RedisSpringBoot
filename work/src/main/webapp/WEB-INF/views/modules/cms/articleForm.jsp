		...
		$(document).ready(function() {
			...
			$("#inputForm").validate({
				submitHandler: function(form){
				 	var contentValue = $('#cke_contents_content').children('iframe').contents().children('html').children('body')
					$('#content').val(contentValue.html())
			...
			<div class="controls">
               ...
               <c:choose>
               		<c:when test="${not empty article.image}">
               			 <input type="hidden" id="image" name="image" value="${ctxStatic }${article.image}"/>
               		</c:when>
               		<c:otherwise>
