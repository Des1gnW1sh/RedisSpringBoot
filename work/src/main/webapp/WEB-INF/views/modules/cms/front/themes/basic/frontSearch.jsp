	...
	<div  class="mainDiv">
		...
		<%-- 	<c:choose>
				...
				<c:otherwise> --%>
					...
					<input type="text" name="content" value="${content}" class="txt"/>
		...
		<dl class="search">
			<c:if test="${empty t || t eq 'article'}">
				<c:forEach items="${page.list}" var="article">
					...
					<c:choose>
						<c:when test="${ not empty article.articleData.content}"><dd>${article.articleData.content}<span class="info"></c:when>
						<c:otherwise><dd>${article.description}<span class="info"></c:otherwise>
					</c:choose>
					<br/>
					发布者：${article.createBy.name} &nbsp; 点击数：${article.hits} &nbsp; 发布时间：<fmt:formatDate value="${article.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/> &nbsp; 更新时间：<fmt:formatDate value="${article.updateDate}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
